...
class Evaluator(_Evaluator):
    ...
    def __init__(self, config, **kwargs):
        RL = functools.reduce(lambda x, wrap: wrap(x), itertools.chain((module.RL,), map(wuji.parse.instance, filter(None, config.get('rl', 'wrap').split('\t')))))
    ...
    def get_hparam_real(self):
        def make(i, upper):
            return dict(
                ...
                set=lambda value: self.rl.weight_loss.__setitem__(i, float(value)),
                get=lambda: np.array([self.rl.weight_loss[i]], np.float),
            )
        hparam = {}
        upper = self.config.getfloat('rl', 'discount_')
        if upper > 0:
            hparam['discount'] = dict(
                boundary=np.array([(0, upper)], np.float),
        ...
        for i, key in enumerate(module.Losses._fields):
            ...
            if upper > 0:
                hparam['weight_' + key] = make(i, upper)
        return hparam
    ...
    def create_recorder(self):
        ...
        recorder.register(self.config.get('record', 'scalar'), lambda: wuji.record.Scalar(self.cost, **{
            ...
            **{f'{NAME}/losses/{field}': getattr(self.outcome.losses, field).item() for field in self.outcome.losses._fields},
        }))
        return recorder
