...
class Evaluator(_Evaluator):
    ...
    def get_hparam_real(self):
        ...
        if upper > 0:
            hparam['prob_min'] = dict(
                ...
                get=lambda: np.array([self.rl.prob_min], np.float),
            )
    ...
    def get_hparam_integer(self):
        hparam = {}
        upper = self.config.getint('train', 'batch_size_')
        if upper > 0:
            hparam['truncation'] = dict(
                boundary=np.array([(1, upper)], np.int),
                set=lambda value: self.rl.set_batch_size(value),
                get=lambda: np.array([self.rl.get_batch_size()], np.int),
            )
        return hparam
